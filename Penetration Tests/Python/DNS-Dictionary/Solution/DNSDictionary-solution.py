...
for index in range(0, len(ips)):
    DNS_dictionary.update({providers[index]: ips[index]})


print("DNS Dictionary: ")
pprint.pprint(DNS_dictionary)
print("--------")

# Use the dictionary to print Hurricane Electric's IP
print("Hurricane Electric's IP is: " + DNS_dictionary.get("Hurricane Electric"))
print("--------")
print("--------")
...
for index in range(0, len(providers)):
    temp_dictionary = {
        ...
        'primary_server': ips[index]
    }
    DNS_dictionaries.append(temp_dictionary)

print("DNS Dictionaries: ")
pp.pprint(DNS_dictionaries)
print("--------")
...
for index in range(0, len(secondary_ips)):
    provider_name = secondary_ips[index].get("provider")
    DNS_dictionary.update({
        provider_name:
        [DNS_dictionary.get(provider_name), secondary_ips[index].get("ip")]
    }
    )
...
for index in range(0, len(secondary_ips)):
    provider_name = secondary_ips[index].get("provider")
    dns = DNS_dictionaries[index]
    if (dns.get("provider_name") == provider_name):
        dns.update({'secondary_server': secondary_ips[index].get("ip")})

print("DNS Dictionary: ")
pprint.pprint(DNS_dictionaries)
print("--------")
